...
if (step == "final") {
    ...
    # Функция для бутстрапа важности (LMG/Anova)
    bt_fn <- function(d, i) {
        m_b <- lm(f_obj, data = d[i,,drop=F])
        an  <- car::Anova(m_b, type="II")
        ss  <- an$`Sum Sq`
        ss_eff <- ss[-length(ss)] # Убираем Residuals
        lmg_shares <- ss_eff / sum(ss_eff)
        return(c(summary(m_b)$r.squared, lmg_shares))
    }

    # Запускаем БУТСТРАП ВАЖНОСТИ
    b_res <- boot::boot(data = df_f, statistic = bt_fn, R = self$options$nboot)
    # 1. Функция расчета (R2 + доли дисперсии по Anova)
     bt_fn <- function(d, i) {
        m_b <- lm(f_obj, data = d[i,,drop=F])
        an  <- car::Anova(m_b, type="II")
        ss  <- an$`Sum Sq`
    ...
    # --- РАСЧЕТ BETA (БЕЗОПАСНЫЙ) ---
    # Масштабируем только те колонки, которые реально есть в df_f
    df_std <- df_f
    for (cn in names(df_std)) {
        if (is.numeric(df_std[[cn]])) df_std[[cn]] <- as.numeric(scale(df_std[[cn]]))
    }
    m_std <- lm(f_obj, data = df_std)
    betas <- coef(m_std)[-1] # Убираем Intercept
    betas[is.na(betas)] <- 0

    # --- ЗАПОЛНЕНИЕ ТАБЛИЦ ---
    m_t <- self$results$metricsTable
    ci_r2 <- try(boot::boot.ci(b_res, type="perc", index=1)$percent[4:5], silent=T)
    m_t$setRow(rowNo=1, values=list(
        metric = "R-squared", 
        value  = b_res$t0[1],
        lower  = if(inherits(ci_r2, "try-error")) NA else ci_r2[1],
        upper  = if(inherits(ci_r2, "try-error")) NA else ci_r2[2]
    ))
...
if (show_facets) {
   ...
   formula_facet <- as.formula(paste0("~ `", mod2_name, "`"))
    canvas <- canvas + 
