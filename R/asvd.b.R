...
if (step == "final") {
    ...
    # 3. Расчет стандартизированных Beta (для колонки Beta в таблице)
    df_std <- data.frame(lapply(df_f, function(x) as.numeric(scale(x))))
    m_std  <- lm(f_obj, data = df_std)
    betas  <- coef(m_std)[-1]
    betas[is.na(betas)] <- 0

    # 4. Заполнение Metric Table
